...
class ACGAN():
    ...
    def train(self, epochs, batch_size=16, save_interval=5):
        ...
        for epoch in range(epochs):
            ...
            # The cluster labels that the generator tries to create
            sampled_labels = np.random.randint(0, 32, half_batch).reshape(-1, 1)
            ...
            fake_labels = 32 * np.ones(half_batch).reshape(-1, 1)
            ...
            # cluster labels
            sampled_labels = np.random.randint(0, 32, batch_size).reshape(-1, 1)
    ...
    def save_imgs(self, epoch):
        ...
        sampled_labels = np.arange(0, 32).reshape(-1, 1)
        ...
        fig.suptitle("ACGAN: Generated clusters", fontsize=12)
        cnt = 0
        for i in range(r):
            for j in range(c):
                ...
                axs[i,j].axis('off')
                cnt += 1
        fig.savefig("C:\\Users\Shivendra\Desktop\GAN\GAN_HCML\images_ACGAN_RC_clusters\%d.png" % epoch)
        plt.close()
...
if __name__ == '__main__':
    acgan = ACGAN()
    acgan.train(epochs=20000, batch_size=16, save_interval=5000)

# def get_data(datadir):
#     X = np.load('C:\\Users\\Shivendra\\Desktop\\GAN\\icon_dataset.npy')
#     Y = np.load('C:\\Users\Shivendra\Desktop\GAN\icon_color_label.npy')
#     split = int(0.8 * len(X))
