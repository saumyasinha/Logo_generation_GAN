...
def Normalize(name, inputs,labels=None):
    ...
    if ('Discriminator' in name) and NORMALIZATION_D:
        return lib.ops.layernorm.Layernorm(name,[1,2,3],inputs,labels=labels,n_labels=32)
    elif ('Generator' in name) and NORMALIZATION_G:
        if labels is not None:
            return lib.ops.cond_batchnorm.Batchnorm(name,[0,2,3],inputs,labels=labels,n_labels=32)
        else:
...
def Discriminator(inputs, labels):
    ...
    if CONDITIONAL and ACGAN:
        output_acgan = lib.ops.linear.Linear('Discriminator.ACGANOutput', DIM_D, 32, output)
...
with tf.Session() as session:
    ...
    for device in DEVICES:
        with tf.device(device):
            ...
            fake_labels = tf.cast(tf.random_uniform([n_samples])*32, tf.int32)
            if CONDITIONAL and ACGAN:
    ...
    def generate_image(frame, true_dist):
        for i in range(32):
            samples = session.run(create_noise(i))
            samples = ((samples+1.)*(255./2)).astype('int32')
            lib.save_images.save_images(samples.reshape((100, 3, 32, 32)), 'C:\\Users\Shivendra\Desktop\GAN\iwgan_acgan_rc_lld_single_cluster\samples_{}.png'.format(i))

    # Function for calculating inception score
    fake_labels_100 = tf.cast(tf.random_uniform([100])*32, tf.int32)
    ...
    for iteration in range(ITERS):
        ...
        if iteration % 1000==999:
            save_path = saver.save(session, "/tmp/model_lc_25k.ckpt", global_step=iteration)
...
#   frame_i = [0]
#   fixed_noise = tf.constant(np.random.normal(size=(10, 128)).astype('float32'))
#   fixed_labels = tf.constant(np.array([0]*10, dtype='int32'))
#   fixed_noise_samples = Generator(10, fixed_labels, noise=fixed_noise)
#   frame=0
#   samples = session.run(fixed_noise_samples)
#   samples = ((samples + 1.) * (255. / 2)).astype('int32')
#   lib.save_images.save_images(samples.reshape((10, 3, 32, 32)),
#                               'C:\\Users\Shivendra\Desktop\GAN\iwgan_acgan_rc_lld_saved_model\samples_{}.png'.format(frame))
#
#
